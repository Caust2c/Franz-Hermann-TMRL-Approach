...
#   - Adjusted JSON input format and spline logic (allows tracks whose start and end arent the same point which is the majority of tm maps)
#
#------------------------------------------------------------------------------+
...
def get_lap_time(racing_line:list, return_all=False):
	...
	us = 1.0 	# Coefficient of friction

	# Computing speed at each point
	MAX_SPEED = 150  
	v = [min(MAX_SPEED, math.sqrt(us * r * 9.81)) for r in radius]
